...
class SolveTSPUsingACO:
    ...
    class Ant:
        ...
        def _select_node(self):
            ...
            for unvisited_node in unvisited_nodes:
                heuristic_total += self.edges[self.tour[-1]
                                              ][unvisited_node].weight
            for unvisited_node in unvisited_nodes:
    ...
    def run(self):
        ...
        if self.mode == 'ACS method':
            self._acs()
        elif self.mode == 'Elitist method':
            self._elitist()
        else:
            self._max_min()
    ...
    def plot(self, line_width=1, point_radius=math.sqrt(2.0), annotation_size=8, dpi=120, save=True, name=None):
        ...
        y.append(y[0])
        plt.plot(x, y, linewidth=line_width, color="green")
        plt.scatter(x, y, s=math.pi * (point_radius ** 2.0), color="red")
        plt.title(self.mode)
...
if __name__ == '__main__':
    _colony_size = 5
    _steps = 50
    _nodes = [(random.uniform(-400, 400), random.uniform(-400, 400))
              for _ in range(0, 200)]
    acs = SolveTSPUsingACO(
        mode='ACS Method', colony_size=_colony_size, steps=_steps, nodes=_nodes)
    acs.run()
    acs.plot()
    elitist = SolveTSPUsingACO(
        mode='Elitist Method', colony_size=_colony_size, steps=_steps, nodes=_nodes)
    elitist.run()
    elitist.plot()
    max_min = SolveTSPUsingACO(
        mode='MaxMin Method', colony_size=_colony_size, steps=_steps, nodes=_nodes)
    max_min.run()
    max_min.plot()
